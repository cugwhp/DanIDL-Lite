FUNCTION calculate_outer_sum_of_three_vectors, vec1, vec2, vec3, status, INTEGER_OUTPUT = integer_output, NO_PAR_CHECK = no_par_check
...
if ((nv2 EQ 1L) AND (nv3 EQ 1L)) then begin
  ...
  ;Calculate and return the outer sum
...
if (nv1 EQ 1L) then begin
  ...
  ;Calculate and return the outer sum
  if keyword_set(integer_output) then begin
    return, reform(calculate_outer_sum_of_two_vectors(vec2, long(vec1[0]) + long(vec3), stat, /INTEGER_OUTPUT, /NO_PAR_CHECK), 1L, nv2, nv3)
  endif else return, reform(calculate_outer_sum_of_two_vectors(vec2, double(vec1[0]) + double(vec3), stat, /NO_PAR_CHECK), 1L, nv2, nv3)
endif
...
if (nv2 EQ 1L) then begin
  ...
  ;Calculate and return the outer sum
  if keyword_set(integer_output) then begin
    return, reform(calculate_outer_sum_of_two_vectors(vec1, long(vec2[0]) + long(vec3), stat, /INTEGER_OUTPUT, /NO_PAR_CHECK), nv1, 1L, nv3)
  endif else return, reform(calculate_outer_sum_of_two_vectors(vec1, double(vec2[0]) + double(vec3), stat, /NO_PAR_CHECK), nv1, 1L, nv3)
endif
...
if (nv3 EQ 1L) then begin
  ...
  ;Calculate and return the outer sum
  if keyword_set(integer_output) then begin
    return, calculate_outer_sum_of_two_vectors(vec1, long(vec3[0]) + long(vec2), stat, /INTEGER_OUTPUT, /NO_PAR_CHECK)
  endif else return, calculate_outer_sum_of_two_vectors(vec1, double(vec3[0]) + double(vec2), stat, /NO_PAR_CHECK)
endif

;The remainder of the code deals with the case where all three vectors each have at least two elements. Calculate
;and return the outer sum.
tmp_vec = reform(calculate_outer_sum_of_two_vectors(vec1, vec2, stat, INTEGER_OUTPUT = integer_output, /NO_PAR_CHECK), nv1*nv2)
return, reform(calculate_outer_sum_of_two_vectors(tmp_vec, vec3, stat, INTEGER_OUTPUT = integer_output, /NO_PAR_CHECK), nv1, nv2, nv3)
