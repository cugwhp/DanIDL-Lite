FUNCTION calculate_outer_sum_of_two_vectors_werr, vec1, vec1_var, vec1_bpm, vec2, vec2_var, vec2_bpm, outdata_var, outdata_bpm, status, $
...
;              independent and the function employs the following equation (see the DanIDL function
;              "propagate_variance_for_linear_combination_pairs.pro"):
;
;              Var(A_ij) = Var(V1_i) + Var(V2_j)
;
...
;              sum are returned via the output parameter "outdata_var". The function provides the option of
...
;              The bad pixel mask corresponding to the outer sum is returned via the output parameter
...
;Calculate the outer sum matrix
outdata = calculate_outer_sum_of_two_vectors(vec1, vec2, status, INTEGER_OUTPUT = integer_output, NO_PAR_CHECK = no_par_check)
...
if ~keyword_set(no_var_propagation) then begin
  ...
  outdata_var = calculate_outer_sum_of_two_vectors(vec1_var_use, vec2_var_use, stat, /NO_PAR_CHECK)
