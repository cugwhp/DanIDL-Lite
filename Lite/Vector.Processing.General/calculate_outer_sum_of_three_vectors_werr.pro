FUNCTION calculate_outer_sum_of_three_vectors_werr, vec1, vec1_var, vec1_bpm, vec2, vec2_var, vec2_bpm, vec3, vec3_var, vec3_bpm, outdata_var, $
...
; Description: This function calculates the outer sum of three input vectors "vec1", "vec2" and "vec3" not
;              necessarily of the same length. The outer sum of three vectors V1, V2 and V3, of length M, N and
;              P elements respectively, is the MxNxP array A defined by:
;
...
;
;              Var(A_ijk) = Var(V1_i) + Var(V2_j) + Var(V3_k)
;
...
;              corresponding to the outer sum are returned via the output parameter "outdata_var". The function
;              provides the option of performing error propagation instead of variance propagation via the use
;              of the keyword ERROR_PROPAGATION. The function also provides the option of not performing
...
;              any other value. The bad pixel mask corresponding to the outer sum is returned via the output
...
;                                                parameter are assumed to be zero. If the keyword
;                                                NO_BPM_PROPAGATION is set, then this input parameter is ignored.
;   outdata_var - ANY - A variable which will be used to contain the variances corresponding to the outer sum on
...
;                       sum on returning (see output parameters below).
...
;                                variances corresponding to the outer sum. If the keyword ERROR_PROPAGATION is
;                                set, then the elements of this output parameter represent the standard errors
;                                corresponding to the outer sum. If the keyword NO_VAR_PROPAGATION is set, then
;                                this output parameter is set to "0.0D".
;   outdata_bpm - INTEGER ARRAY - An array of the same size as the return value where the elements represent the
;                                 bad pixel mask values corresponding to the outer sum. A value of "1" flags a
;                                 good pixel, and a value of "0" flags a bad pixel. If the keyword
...
;   represents the outer sum of the three vectors "vec1", "vec2" and "vec3". Note that if "vec2" and "vec3" are
;   both scalar numbers or single-element one-dimensional vectors, then the function returns an M-element vector
;   of DOUBLE type numbers since IDL treats an Mx1x1 array as an M-element one-dimensional vector. Also, if only
;   "vec3" is a scalar number or a single-element one-dimensional vector, then the function returns an M by N
;   element two-dimensional array of DOUBLE type numbers since IDL treats an MxNx1 array as an MxN-element
...
;   the outer sum array as a three-dimensional array of LONG type integers instead of DOUBLE type numbers.
;
...
;Calculate the outer sum array
outdata = calculate_outer_sum_of_three_vectors(vec1, vec2, vec3, status, INTEGER_OUTPUT = integer_output, NO_PAR_CHECK = no_par_check)
...
if ~keyword_set(no_var_propagation) then begin
  ...
  outdata_var = calculate_outer_sum_of_three_vectors(vec1_var_use, vec2_var_use, vec3_var_use, stat, /NO_PAR_CHECK)
