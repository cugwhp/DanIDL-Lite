FUNCTION arcsec2rad_werr, arcsec, arcsec_var, rad_var, status, ERROR_PROPAGATION = error_propagation, NO_PAR_CHECK = no_par_check

; Description: This function converts angles in arcseconds "arcsec" to angles in radians. The input parameter
;              "arcsec" may be a scalar, vector, or array.
...
;   "arcsec", where each element represents an angle in units of radians.
;
; Keywords:
;
...
;   "arcsec_var" are assumed to represent standard errors, and the output parameter "rad_var" is returned
...
rad = arcsec2rad(arcsec, status, NO_PAR_CHECK = no_par_check)
...
if (arcsec_var_tag EQ 1) then begin
  if keyword_set(error_propagation) then begin
    rad_var = (get_dbl_pi()/648000.0D)*arcsec_var
  endif else rad_var = (get_dbl_pi2()/419904000000.0D)*arcsec_var
  if (arcsec.ndim EQ 0L) then begin
    rad_var = rad_var[0]
  endif else rad_var = reform(rad_var, arcsec.dim, /OVERWRITE)
endif else begin
  if (arcsec.ndim GT 0L) then rad_var = dblarr(arcsec.dim)
endelse
