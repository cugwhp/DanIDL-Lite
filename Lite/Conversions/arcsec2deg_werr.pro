FUNCTION arcsec2deg_werr, arcsec, arcsec_var, deg_var, status, ERROR_PROPAGATION = error_propagation, NO_PAR_CHECK = no_par_check

; Description: This function converts angles in arcseconds "arcsec" to angles in degrees. The input parameter
...
;   "arcsec", where each element represents an angle in units of degrees.
;
; Keywords:
;
...
;   "arcsec_var" are assumed to represent standard errors, and the output parameter "deg_var" is returned
...
deg = arcsec2deg(arcsec, status, NO_PAR_CHECK = no_par_check)
...
if (arcsec_var_tag EQ 1) then begin
  if keyword_set(error_propagation) then begin
    deg_var = (1.0D/3600.0D)*arcsec_var
  endif else deg_var = (1.0D/12960000.0D)*arcsec_var
  if (arcsec.ndim EQ 0L) then begin
    deg_var = deg_var[0]
  endif else deg_var = reform(deg_var, arcsec.dim, /OVERWRITE)
endif else begin
  if (arcsec.ndim GT 0L) then deg_var = dblarr(arcsec.dim)
endelse
