FUNCTION arcmin2arcsec_werr, arcmin, arcmin_var, arcsec_var, status, ERROR_PROPAGATION = error_propagation, NO_PAR_CHECK = no_par_check

; Description: This function converts angles in arcminutes "arcmin" to angles in arcseconds. The input
...
;   "arcmin", where each element represents an angle in units of arcseconds.
;
; Keywords:
;
...
;   "arcmin_var" are assumed to represent standard errors, and the output parameter "arcsec_var" is returned
...
arcsec = arcmin2arcsec(arcmin, status, NO_PAR_CHECK = no_par_check)
...
if (arcmin_var_tag EQ 1) then begin
  if keyword_set(error_propagation) then begin
    arcsec_var = 60.0D*arcmin_var
  endif else arcsec_var = 3600.0D*arcmin_var
  if (arcmin.ndim EQ 0L) then begin
    arcsec_var = arcsec_var[0]
  endif else arcsec_var = reform(arcsec_var, arcmin.dim, /OVERWRITE)
endif else begin
  if (arcmin.ndim GT 0L) then arcsec_var = dblarr(arcmin.dim)
endelse
