FUNCTION rev2deg_werr, rev, rev_var, deg_var, status, ERROR_PROPAGATION = error_propagation, NO_PAR_CHECK = no_par_check

; Description: This function converts numbers of revolutions "rev" to angles in degrees. The input
...
;              corresponding to the angles in degrees are returned via the output parameter "deg_var".
...
;   deg_var - ANY - A variable which will be used to contain the variances corresponding to the angles
;                   in units of degrees on returning (see output parameters below).
...
;   "rev", where each element represents an angle in units of degrees.
;
; Keywords:
;
...
;   parameter "rev_var" are assumed to represent standard errors, and the output parameter "deg_var"
...
;Convert the input numbers of revolutions to angles in units of degrees
deg = rev2deg(rev, status, NO_PAR_CHECK = no_par_check)
...
if (rev_var_tag EQ 1) then begin
  if keyword_set(error_propagation) then begin
    deg_var = 360.0D*rev_var
  endif else deg_var = 129600.0D*rev_var
  if (rev.ndim EQ 0L) then begin
    deg_var = deg_var[0]
  endif else deg_var = reform(deg_var, rev.dim, /OVERWRITE)
endif else begin
  if (rev.ndim GT 0L) then deg_var = dblarr(rev.dim)
endelse

;Return the input numbers of revolutions in units of degrees
return, deg
