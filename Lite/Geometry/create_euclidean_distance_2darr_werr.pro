PRO create_euclidean_distance_2darr_werr, xsize, ysize, x0, x0_var, y0, y0_var, dist, dist_var, status, SQUARED_DISTANCE = squared_distance, $
                                          ERROR_PROPAGATION = error_propagation, NO_PAR_CHECK = no_par_check

; Description: This module creates a two-dimensional pixel array "dist" of size "xsize" by "ysize" pixels with
;              values representing the Euclidean distance of each pixel from the pixel coordinates "(x0,y0)".
;                The module also provides the option of creating a two-dimensional pixel array "dist" of size
;              "xsize" by "ysize" pixels with values representing the square of the Euclidean distance of each
;              pixel from the pixel coordinates "(x0,y0)" through the use of the keyword SQUARED_DISTANCE.
;                The module also performs propagation of the variances "x0_var" and "y0_var" associated with
;              the input parameters "x0" and "y0", respectively. It is assumed that the input parameters are
;              independent. The variance propagation that is performed is approximate (using linearisation by
;              first-order Taylor series expansion), which is especially important to consider for the pixels
;              with coordinates close to "(x0,y0)". The variances corresponding to the distance array "dist"
;              are returned via the output parameter "dist_var". The module provides the option of performing
;              error propagation instead of variance propagation via the use of the keyword ERROR_PROPAGATION.
;                The module does not perform bad pixel mask propagation since it does not make sense for a
;              module with two relevant single-element input parameters "x0" and "y0".
;
; Input Parameters:
;
;   xsize - INTEGER/LONG - The size (pix) of the two-dimensional pixel array "dist" in the x direction. This
;                          parameter must be positive.
;   ysize - INTEGER/LONG - The size (pix) of the two-dimensional pixel array "dist" in the y direction. This
;                          parameter must be positive.
;   x0 - FLOAT/DOUBLE - The x coordinate (pix) of the point from which the Euclidean distances are to be
;                       calculated.
;   x0_var - FLOAT/DOUBLE - The variance of the input parameter "x0". This parameter should be non-negative.
;                           If this input parameter does not satisfy the input requirements, then the variance
;                           is assumed to be zero. If the keyword ERROR_PROPAGATION is set, then this input
...
;                       calculated.
;   y0_var - FLOAT/DOUBLE - The variance of the input parameter "y0". This parameter should be non-negative.
;                           If this input parameter does not satisfy the input requirements, then the variance
;                           is assumed to be zero. If the keyword ERROR_PROPAGATION is set, then this input
;                           parameter is assumed to represent the standard error as opposed to the variance.
;
...
;                         "(x0,y0)". If the keyword SQUARED_DISTANCE is set, then the values in this output
;                         parameter represent the square of the Euclidean distance of each pixel from the
;                         pixel coordinates "(x0,y0)". Note that if "ysize" is set to "1", then this parameter
;                         is returned as a one-dimensional vector of length "xsize" pixels since IDL treats an
;                         Nx1 array as an N-element one-dimensional vector.
...
;   of the Euclidean distance of each pixel from the pixel coordinates "(x0,y0)" instead of calculating the
;   Euclidean distance.
;
;   If the keyword ERROR_PROPAGATION is set (as "/ERROR_PROPAGATION"), then the module will perform error
;   propagation instead of variance propagation. In other words, the input parameters "x0_var" and "y0_var"
;   are assumed to represent standard errors, and the output parameter "dist_var" is returned with elements
...
;If the keyword ERROR_PROPAGATION is set, then square the input parameters "x0_var" and "y0_var"
if keyword_set(error_propagation) then begin
  x0_var_use = x0_var_use*x0_var_use
  y0_var_use = y0_var_use*y0_var_use
endif

;Generate a two-dimensional pixel array representing the squared Euclidean distances from "(x0,y0)" and perform
;the variance propagation
...
dist = calculate_outer_sum_of_two_vectors_werr(xd2, xd2_var, xd2_bpm, yd2, yd2_var, yd2_bpm, dist_var, dist_bpm, stat, /NO_BPM_PROPAGATION, $
                                               /NO_PAR_CHECK)
...
;If the keyword SQUARED_DISTANCE is not set
  ...
  if (nsubs GT 0L) then begin
    ...
    ;greatest value from the input parameters "x0_var" and "y0_var", which is a conservative estimate of the
    ;output variance.
    dist[subs] = 0.25D
    dist_var[subs] = x0_var_use > y0_var_use
