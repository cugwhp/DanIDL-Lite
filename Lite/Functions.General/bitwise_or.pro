FUNCTION bitwise_or, bit_pattern_data, status, danidl_cppcode

; Description: This function performs a bit-wise OR operation between all elements of the input set of integers
...
;   result of performing a bit-wise OR operation between all the elements of "bit_pattern_data".
;
...
;If the input parameter "bit_pattern_data" has only one element, then set the return value of this function to
;the value of the single element of "bit_pattern_data"
...
if (danidl_cppstat EQ 1) then begin
  ...
  ;Combine the elements of "bit_pattern_data" using the bit-wise OR operator
  bit_pattern_result = 0L
  result = call_external(danidl_cpp_shared_lib, 'bitwise_or', ndata, long(bit_pattern_data), bit_pattern_result)
...
;If the shared library of DanIDL C++ routines is not installed, then use the default IDL code
  ...
  ;to combine the elements of "bit_pattern_data" using the bit-wise OR operator
  bit_pattern_result = bit_pattern_data
  while (ndata GT 1L) do begin

    ;If the current number of elements still to be combined using the bit-wise OR operator is odd
    if ((ndata mod 2L) EQ 1L) then begin

      ;Perform a bit-wise OR operation between the first and last element of the current set of elements still to
      ;be combined
      ndata = ndata - 1L
      bit_pattern_result[0] = bit_pattern_result[0] OR bit_pattern_result[ndata]
    endif

    ;Perform a bit-wise OR operation between each pair of elements still to be combined by employing a vector
    ;operation
    hndata = ndata/2L
    bit_pattern_result = bit_pattern_result[0L:(hndata - 1L)] OR bit_pattern_result[hndata:(ndata - 1L)]

    ;Update the number of elements still to be combined using the bit-wise OR operator
    ndata = hndata
  endwhile
