FUNCTION grow_bit_pattern_bpm_2darr, bpmdata, grad, status, SQUARE = square, BPM_BEYOND_EDGE = bpm_beyond_edge, NO_PAR_CHECK = no_par_check
...
;              bad pixel mask "bpmdata" by a radius "grad" (in pixel units). The user also
;              has the alternative option of growing each bit that is set in each pixel of
;              the input bad pixel mask "bpmdata" by the growth distance "grad" pixels in
;              each of the x and y coordinate directions if so required.
...
;              growth operation, and this functionality is provided by the function through
...
;   grad - FLOAT/DOUBLE - The radius in pixels by which to grow each bit that is set in
...
;   numbers of the same size as "bpmdata". The return array is the result of growing each
;   bit that is set in each pixel of the input bad pixel mask "bpmdata" by a radius of
;   "grad" pixels.
;
; Keywords:
;
;   If the keyword SQUARE is set (as "/SQUARE"), then instead of growing each bit that is
;   set in each pixel of the input bad pixel mask by a growth radius, the bits are grown
;   by a distance of "grad" pixels along each of the x and y coordinate directions.
;
...
;Determine the convolution kernel required to grow the bits that are set in each pixel
;of the input bad pixel mask
hksize = floor(grad)
ksize = (2L*hksize) + 1L
if keyword_set(square) then begin
  ...
  krad = 0.5D*ksize
  kernel = long(create_circular_mask_2darr(ksize, ksize, krad, krad, grad, stat, /NO_PAR_CHECK))
endelse

;Grow each bit that is set in each pixel of the input bad pixel mask "bpmdata" by a radius
;of "grad" pixels and return the result
return, transform_bit_pattern_bpm_2darr_with_kernel(bpmdata, kernel, status, BPM_BEYOND_EDGE = bpm_beyond_edge, /NO_PAR_CHECK)
