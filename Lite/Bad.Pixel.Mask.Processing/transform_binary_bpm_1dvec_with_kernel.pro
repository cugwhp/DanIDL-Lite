FUNCTION transform_binary_bpm_1dvec_with_kernel, bpmdata, kernel, status, GROW_GOOD_PIX = grow_good_pix, BAD_BEYOND_EDGE = bad_beyond_edge, NO_PAR_CHECK = no_par_check

; Description: This function processes a one-dimensional binary bad pixel mask "bpmdata", where the term
...
;              in the resulting vector are set to good pixels with a value of "1" in the output bad
;              pixel mask, and non-zero pixel values in the resulting vector are set to bad pixels with
;              a value of "0" in the output bad pixel mask.
...
;              convolution, the length of the kernel must be an odd number. Kernels not satisfying this
...
;              the input kernel "kernel". Pixel values of "0" in the resulting vector are set to bad
;              pixels with a value of "0" in the output bad pixel mask, and non-zero pixel values in the
;              resulting vector are set to good pixels with a value of "1" in the output bad pixel mask.
...
;              pixel mask, the function assumes that the pixels are good beyond the ends of the input bad
;              pixel mask. However, the user may wish to assume that the pixels are bad beyond the ends
...
; Input Parameters:
;
;   bpmdata - BYTE/INTEGER/LONG VECTOR - A one-dimensional bad pixel mask vector which flags good pixels
;                                        with a value of "1" and bad pixels with any other value.
;   kernel - BYTE/INTEGER/LONG VECTOR - A one-dimensional kernel vector which is to be used to transform
;                                       the bad pixels in the bad pixel mask using the convolution
;                                       operation defined above. The length of the kernel vector must be
;                                       an odd number since the kernel will be centred over each pixel
;                                       in the bad pixel mask during the convolution. If this is not the
;                                       case, then the function will fail. Usually kernels for
;                                       transforming the bad pixels in the input bad pixel mask consist
;                                       solely of the values "1" and "0".
...
; Return Value:
;
;   The function returns a one-dimensional bad pixel mask vector of INTEGER type numbers of the same length
;   as "bpmdata" which flags good pixels with a value of "1" and bad pixels with a value of "0". The return
;   vector is the result of having transformed the input bad pixel mask using the convolution operation
;   defined above.
;
; Keywords:
;
...
;   are bad beyond the ends of the input bad pixel mask for the purposes of the convolution operation.
;
...
if keyword_set(no_par_check) then begin
  ...
  ;Determine the length of "bpmdata"
...
if (kxsize EQ 1L) then begin
  ...
  if (kernel_use[0] EQ 0L) then begin
    ...
    if keyword_set(grow_good_pix) then begin
      bpm_out = intarr(bpmxsize)
    endif else bpm_out = replicate(1, bpmxsize)
...
;Create an internal-format version of the input bad pixel mask vector which flags good pixels with a value
;of "1" and bad pixels with a value of "0"
...
if keyword_set(bad_beyond_edge) then begin
  bpmdata_padded = lonarr(bpmxsize_m1 + kxsize)
endif else bpmdata_padded = replicate(1L, bpmxsize_m1 + kxsize)
bpmdata_padded[hkxsize] = bpm_out
...
return, bpm_out[hkxsize:(bpmxsize_m1 + hkxsize)]
