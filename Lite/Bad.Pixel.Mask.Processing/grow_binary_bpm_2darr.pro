FUNCTION grow_binary_bpm_2darr, bpmdata, grad, status, SQUARE = square, BAD_BEYOND_EDGE = bad_beyond_edge, NO_PAR_CHECK = no_par_check
...
;              radius "grad" (in pixel units). This means that any pixel in the output
...
;              mask "bpmdata" will be flagged as bad. The user also has the alternative
;              option of growing bad pixels by the growth distance "grad" pixels in each
;              of the x and y coordinate directions if so required.
...
;   pixels with a value of "0". The return array is the result of growing the bad pixels
;   in the input bad pixel mask "bpmdata" by a radius of "grad" pixels.
;
; Keywords:
;
;   If the keyword SQUARE is set (as "/SQUARE"), then instead of growing bad pixels in the
;   input bad pixel mask by a growth radius, the bad pixels are grown by a distance of
;   "grad" pixels along each of the x and y coordinate directions.
;
...
;Determine the convolution kernel required to grow the bad pixels in the input bad pixel
;mask
hksize = floor(grad)
ksize = (2L*hksize) + 1L
...
;return the result
return, transform_binary_bpm_2darr_with_kernel(bpmdata, kernel, status, BAD_BEYOND_EDGE = bad_beyond_edge, /NO_PAR_CHECK)
