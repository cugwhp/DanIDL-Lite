...
;              the integer value is positive, and this is generally considered to be a bad
;              pixel with the "bad pixel" properties indicated by which bits are set.
...
;              of BYTE, INTEGER, or LONG type, and that it contains only non-negative pixel
;              values.
;
; Input Parameters:
;
;   bpmdata - ANY - The parameter to be tested whether or not it satisfies the properties
;                   that a three-dimensional bit-pattern bad pixel mask should have.
...
;   that a three-dimensional bit-pattern bad pixel mask should have. Otherwise, the function
...
;that a three-dimensional bit-pattern bad pixel mask should have, then return a value of "1".
