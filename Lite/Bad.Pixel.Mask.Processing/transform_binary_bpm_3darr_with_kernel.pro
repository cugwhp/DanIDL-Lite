FUNCTION transform_binary_bpm_3darr_with_kernel, bpmdata, kernel, status, GROW_GOOD_PIX = grow_good_pix, BAD_BEYOND_EDGE = bad_beyond_edge, NO_PAR_CHECK = no_par_check

; Description: This function processes a three-dimensional binary bad pixel mask "bpmdata", where the term
...
;              convolution, the size of the kernel in each of the x, y and z coordinate directions must
;              be an odd number. Kernels not satisfying this criterion will be rejected.
...
;              with a value of "0" in the output bad pixel mask, and non-zero pixel values in the
;              resulting array are set to good pixels with a value of "1" in the output bad pixel mask.
;              This option is intended to be used for "growing" the good pixels in the input bad pixel
;              mask by an arbitrary shape that is defined by the input kernel "kernel".
;                In order to be able to perform the convolution operation at the edges of the input bad
;              pixel mask, the function assumes that the pixels are good beyond the edges of the input
;              bad pixel mask. However, the user may wish to assume that the pixels are bad beyond the
;              edges of the input bad pixel mask for the purposes of the convolution operation, and this
...
;                   "kernel" must only be supplied as either a one-dimensional vector or a
;                   three-dimensional array.
;
; Input Parameters:
;
;   bpmdata - BYTE/INTEGER/LONG ARRAY - A three-dimensional bad pixel mask array which flags good pixels
;                                       with a value of "1" and bad pixels with any other value.
;   kernel - BYTE/INTEGER/LONG VECTOR/ARRAY - A one- or three-dimensional kernel array which is to be
;                                             used to transform the bad pixels in the bad pixel mask
;                                             using the convolution operation defined above. The size of
;                                             each dimension of the kernel array must be an odd number
;                                             since the kernel will be centred over each pixel in the bad
;                                             pixel mask during the convolution. If this is not the case,
;                                             then the function will fail. Usually kernels for transforming
;                                             the bad pixels in the input bad pixel mask consist solely of
;                                             the values "1" and "0".
...
; Return Value:
;
;   The function returns a three-dimensional bad pixel mask array of INTEGER type numbers of the same size
;   as "bpmdata" which flags good pixels with a value of "1" and bad pixels with a value of "0". The return
;   array is the result of having transformed the input bad pixel mask using the convolution operation
;   defined above.
;
; Keywords:
;
...
;If parameter checking is required
  ...
  ndim = kernel.ndim
  if ((ndim NE 1L) AND (ndim NE 3L)) then return, 0
endelse
...
if ((kxsize EQ 1L) AND (kysize EQ 1L) AND (kzsize EQ 1L)) then begin
  ...
  if (kernel_use[0] EQ 0L) then begin
    ...
    if keyword_set(grow_good_pix) then begin
      bpm_out = intarr(bpmxsize, bpmysize, bpmzsize)
    endif else bpm_out = replicate(1, bpmxsize, bpmysize, bpmzsize)
...
bpmzsize_m1 = bpmzsize - 1L
if keyword_set(bad_beyond_edge) then begin
  bpmdata_padded = lonarr(bpmxsize_m1 + kxsize, bpmysize_m1 + kysize, bpmzsize_m1 + kzsize)
endif else bpmdata_padded = replicate(1L, bpmxsize_m1 + kxsize, bpmysize_m1 + kysize, bpmzsize_m1 + kzsize)
bpmdata_padded[hkxsize,hkysize,hkzsize] = bpm_out
...
return, bpm_out[hkxsize:(bpmxsize_m1 + hkxsize), hkysize:(bpmysize_m1 + hkysize), hkzsize:(bpmzsize_m1 + hkzsize)]
