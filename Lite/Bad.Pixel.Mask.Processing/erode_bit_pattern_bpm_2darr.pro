FUNCTION erode_bit_pattern_bpm_2darr, bpmdata, erad, status, SQUARE = square, BPM_BEYOND_EDGE = bpm_beyond_edge, NO_PAR_CHECK = no_par_check

; Description: This function processes a two-dimensional bit-pattern bad pixel mask "bpmdata",
...
;              mask. This is equivalent to eroding the edges of each connected area of pixels
;              with the same bit set in the input bad pixel mask "bpmdata" by a radius of
;              "erad" (in pixel units). The user also has the alternative option of eroding
;              these areas of connected pixels with the same bit set in the input bad pixel
;              mask "bpmdata" by the erosion distance "erad" pixels in each of the x and y
;              coordinate directions if so required.
...
;              that the pixel values beyond the edges of the input bad pixel mask are set
;              to some other non-negative integer value for the purposes of the bit-pattern
;              erosion operation, and this functionality is provided by the function through
;              the use of the keyword BPM_BEYOND_EDGE.
;
; Input Parameters:
;
;   bpmdata - BYTE/INTEGER/LONG ARRAY - A two-dimensional bad pixel mask array which
...
;   erad - FLOAT/DOUBLE - The radius in pixels by which to erode the edges of each
;                         connected area of pixels with the same bit set in the input bad
;                         pixel mask "bpmdata". This parameter must be non-negative.
...
;                      pixel values of the input bad pixel mask "bpmdata", then "status"
;                      is returned with a value of "1", otherwise it is returned with a
;                      value of "0".
;
; Return Value:
;
;   The function returns a two-dimensional bit-pattern bad pixel mask array of LONG type
;   numbers of the same size as "bpmdata". The return array is the result of eroding the
;   edges of each connected area of pixels with the same bit set in the input bad pixel
;   mask "bpmdata" by a radius of "erad" pixels.
;
; Keywords:
;
;   If the keyword SQUARE is set (as "/SQUARE"), then instead of eroding the edges of each
;   connected area of pixels with the same bit set in the input bad pixel mask by an erosion
;   radius, the edges are eroded by a distance of "erad" pixels along each of the x and y
;   coordinate directions.
;
;   If the keyword BPM_BEYOND_EDGE is set to a non-negative BYTE/INTEGER/LONG value, then
;   the function assumes that the pixel values beyond the edges of the input bad pixel mask
...
;If "erad" is less than "1.0", then the output bad pixel mask will be the same as the
...
;pixels with the same bit set in the input bad pixel mask
hksize = floor(erad)
ksize = (2L*hksize) + 1L
...
;Erode the edges of each connected area of pixels with the same bit set in the input bad
;pixel mask "bpmdata" by a radius of "erad" pixels and return the result
return, transform_bit_pattern_bpm_2darr_with_kernel(bpmdata, kernel, status, /BITWISE_AND, BPM_BEYOND_EDGE = bpm_beyond_edge, /NO_PAR_CHECK)
