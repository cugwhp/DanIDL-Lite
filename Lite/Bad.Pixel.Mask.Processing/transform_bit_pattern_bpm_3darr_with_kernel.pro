FUNCTION transform_bit_pattern_bpm_3darr_with_kernel, bpmdata, kernel, status, BITWISE_AND = bitwise_and, BPM_BEYOND_EDGE = bpm_beyond_edge, NO_PAR_CHECK = no_par_check

; Description: This function processes a three-dimensional bit-pattern bad pixel mask "bpmdata", which stores
;              an integer value at each pixel that represents the pattern of bits that are set (or the equivalent
...
;
;                             [                         ]
;              B'_ijk = OR    [ K_uvw * B_(i+u,j+v,k+w) ]           for all non-zero K_uvw
;                       u,v,w [                         ]
;
;              where B_ijk and B'_ijk represent the pixel values of the input and output bad pixel masks,
;              respectively, for the pixel in the ith column and jth row of the kth two-dimensional slice.
;              The operator OR represents the bit-wise OR operator, and it is performed over the set of
;              kernel pixels with non-zero values K_uvw. If all of the kernel pixel values are zero, then
;              all of the pixels in the output bad pixel mask are set to zero. Note that the kernel pixel
;              indices u, v and w are defined such that the central kernel pixel has (u,v,w) pixel indices
;              of (0,0,0).
...
;              the size of the kernel in each of the x, y and z coordinate directions must be an odd number.
...
;
;              N.B: Due to the way that the convolution function "convol" in IDL works, the input kernel
;                   "kernel" must only be supplied as either a one-dimensional vector or a three-dimensional
;                   array.
;
; Input Parameters:
;
;   bpmdata - BYTE/INTEGER/LONG ARRAY - A three-dimensional bad pixel mask array which stores an integer
...
;                                       pixel. If at least one bit is set, then the integer value is positive,
;                                       and this is generally considered to be a bad pixel with the "bad
;                                       pixel" properties indicated by which bits are set. All elements in
;                                       this array must be non-negative.
;   kernel - BYTE/INTEGER/LONG VECTOR/ARRAY - A one- or three-dimensional kernel array which is to be used
;                                             to transform the bits that are set in the pixels of the input
;                                             bad pixel mask using the "convolution" operation defined above.
...
; Return Value:
;
;   The function returns a three-dimensional bit-pattern bad pixel mask array of LONG type numbers of the
...
if ~keyword_set(no_par_check) then begin
  ...
  ;Check that "kernel" is a one-dimensional vector, or a three-dimensional array, of the correct number type
  if (test_bytintlon(kernel) NE 1) then return, 0L
  ndim = kernel.ndim
  if ((ndim NE 1L) AND (ndim NE 3L)) then return, 0L
endif
result = bpmdata.dim
bpmxsize = result[0]
bpmysize = result[1]
...
if ((kxsize EQ 1L) AND (kysize EQ 1L) AND (kzsize EQ 1L)) then begin

  ;Create the output bad pixel mask array
  kval = kernel_use[0]
  if (kval EQ 0L) then begin
    bpm_out = lonarr(bpmxsize, bpmysize, bpmzsize)
...
bpmzsize_padz = bpmzsize + padz
bpmxsize_padx_m1 = bpmxsize_padx - 1L
bpmysize_pady_m1 = bpmysize_pady - 1L
bpmzsize_padz_m1 = bpmzsize_padz - 1L
bpmdata_padded = replicate(bpm_beyond_edge_use, bpmxsize_padx, bpmysize_pady, bpmzsize_padz)
bpmdata_padded[hkxsize,hkysize,hkzsize] = long(bpmdata)

;Set up a double-padded output bad pixel mask array
bpm_out = lonarr(bpmxsize_padx + padx, bpmysize_pady + pady, bpmzsize_padz + padz)

;Reverse each of the kernel dimensions, which is necessary for the "convolution" method implemented in this
;function
if (kzsize GT 1L) then begin
  kernel_use = reverse(reverse(reverse(kernel_use, 1, /OVERWRITE), 2, /OVERWRITE), 3, /OVERWRITE)
endif else kernel_use = reverse(kernel_use, /OVERWRITE)
...
;Determine the kernel pixel indices along the kernel u-, v- and w-axes for the non-zero kernel pixels
convert_3darr_subscripts_to_pixel_indices, ksubs, kxsize, kysize, kxind, kyind, kzind, nksubs, stat, /NO_PAR_CHECK
...
if (ckval EQ 1L) then begin
  bpm_out[kxind[0],kyind[0],kzind[0]] = bpmdata_padded
endif else bpm_out[kxind[0],kyind[0],kzind[0]] = ckval*bpmdata_padded
...
if (nksubs GT 1L) then begin
  ...
  for i = 1L,(nksubs - 1L) do begin
    ...
    ;Extract the kernel pixel indices along the kernel u-, v- and w-axes for the current non-zero kernel
    ;pixel
    ckxind = kxind[i]
    ckyind = kyind[i]
    ckzind = kzind[i]
    ...
    if keyword_set(bitwise_and) then begin
      if (ckval EQ 1L) then begin
        bpm_out[ckxind,ckyind,ckzind] = bpm_out[ckxind:(ckxind + bpmxsize_padx_m1),ckyind:(ckyind + bpmysize_pady_m1),ckzind:(ckzind + bpmzsize_padz_m1)] AND bpmdata_padded
      endif else bpm_out[ckxind,ckyind,ckzind] = bpm_out[ckxind:(ckxind + bpmxsize_padx_m1),ckyind:(ckyind + bpmysize_pady_m1),ckzind:(ckzind + bpmzsize_padz_m1)] AND $
                                                 (ckval*bpmdata_padded)
    endif else begin
      if (ckval EQ 1L) then begin
        bpm_out[ckxind,ckyind,ckzind] = bpm_out[ckxind:(ckxind + bpmxsize_padx_m1),ckyind:(ckyind + bpmysize_pady_m1),ckzind:(ckzind + bpmzsize_padz_m1)] OR bpmdata_padded
      endif else bpm_out[ckxind,ckyind,ckzind] = bpm_out[ckxind:(ckxind + bpmxsize_padx_m1),ckyind:(ckyind + bpmysize_pady_m1),ckzind:(ckzind + bpmzsize_padz_m1)] OR $
                                                 (ckval*bpmdata_padded)
    endelse
  endfor
endif
...
return, bpm_out[padx:bpmxsize_padx_m1, pady:bpmysize_pady_m1, padz:bpmzsize_padz_m1]
