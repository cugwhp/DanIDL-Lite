PRO symmetric_point_match_1d, x1, x2, tol, nmatch, subs1, subs2, dist, X1_ALREADY_SORTED = x1_already_sorted, X2_ALREADY_SORTED = x2_already_sorted, $
...
;              one-dimensional points. A pair of one-dimensional points P and Q from each coordinate list
...
;                         corresponding to the matched points. This vector has "nmatch" elements, and
;                         each element in "subs1" corresponds in order with the subscripts of the
;                         matching points from the second coordinate list "subs2".
;   subs2 - LONG VECTOR - A one-dimensional vector of subscripts from the second coordinate list
;                         corresponding to the matched points. This vector has "nmatch" elements, and
;                         each element in "subs2" corresponds in order with the subscripts of the
;                         matching points from the first coordinate list "subs1".
;   dist - DOUBLE VECTOR - A one-dimensional vector of Euclidean distances between the matched points.
;                          This vector has "nmatch" elements, and each element in "dist" corresponds
;                          in order with the subscripts stored in the output vectors "subs1" and
;                          "subs2".
;
; Keywords:
;
...
if ~keyword_set(no_par_check) then begin
  ...
  ;Check that "x1" and "x2" are of the correct number type
...
if (nsubs GT 0L) then begin
  for i = 0L,(nsubs - 1L) do begin
    ...
    ;list within the specified distance tolerance
    csub = subs[i]
    clo_ind = lo_ind[csub]
    d12[csub] = min(abs(x1_use[csub] - x2_use[clo_ind:hi_ind[csub]]), min_sub)
...
if (nsubs GT 0L) then begin
  for i = 0L,(nsubs - 1L) do begin
    ...
    csub = subs[i]
    clo_ind = lo_ind[csub]
    min_dist = min(abs(x2_use[csub] - x1_use[clo_ind:hi_ind[csub]]), min_sub)
    s21[csub] = clo_ind + min_sub
  endfor
endif

;If there is at least one point in the first coordinate list for which the closest point in the second coordinate
;list is within the specified distance tolerance "tol"
tmp_subs1 = where(s12 GE 0L, ntmp_subs1)
if (ntmp_subs1 GT 0L) then begin

  ;Determine which pairs of points P and Q from each coordinate list satisfy P being the closest point to Q,
  ;and Q being the closest point to P, and that have a Euclidean distance between them of less than or equal
  ;to the specified distance tolerance "tol"
...
;If there are no points in the first coordinate list for which the closest point in the second coordinate list
;is within the specified distance tolerance "tol", then there are no matching coordinate pairs between the two
;coordinate lists
