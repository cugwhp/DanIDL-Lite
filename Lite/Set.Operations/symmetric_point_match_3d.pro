PRO symmetric_point_match_3d, x1, y1, z1, x2, y2, z2, tol, nmatch, subs1, subs2, dist, X1_ALREADY_SORTED = x1_already_sorted, X2_ALREADY_SORTED = x2_already_sorted, $
...
;              three-dimensional points. A pair of three-dimensional points P and Q from each coordinate
;              list are considered a match if P is the closest point to Q, and Q is the closest point
;              to P (symmetric point matching), and the Euclidean distance between P and Q is less than
;              or equal to "tol". A binary search algorithm is used for speed and efficiency. The module
...
;                                           of elements as "x1".
;   z1 - FLOAT/DOUBLE SCALAR/VECTOR/ARRAY - A scalar/vector/array of z coordinates for the points in the
;                                           first coordinate list. This parameter must have the same number
;                                           of elements as "x1".
...
;   z2 - FLOAT/DOUBLE SCALAR/VECTOR/ARRAY - A scalar/vector/array of z coordinates for the points in the
;                                           second coordinate list. This parameter must have the same
;                                           number of elements as "x2".
...
;If parameter checking is required
  ...
  ;Check that "x1", "y1", and "z1" are of the correct number type
  if (test_fltdbl(x1) NE 1) then return
  if (test_fltdbl(y1) NE 1) then return
  if (test_fltdbl(z1) NE 1) then return

  ;Check that "y1" and "z1" have the same number of elements as "x1"
  nx1 = x1.length
  if (y1.length NE nx1) then return
  if (z1.length NE nx1) then return

  ;Check that "x2", "y2", and "z2" are of the correct number type
  if (test_fltdbl(x2) NE 1) then return
  if (test_fltdbl(y2) NE 1) then return
  if (test_fltdbl(z2) NE 1) then return

  ;Check that "y2" and "z2" have the same number of elements as "x2"
  nx2 = x2.length
  if (y2.length NE nx2) then return
  if (z2.length NE nx2) then return
...
if (nsubs GT 0L) then begin
  for i = 0L,(nsubs - 1L) do begin
    ...
    clo_ind = lo_ind[csub]
    chi_ind = hi_ind[csub]
    min_dist2 = min(((x1_use[csub] - x2_use[clo_ind:chi_ind])^2) + ((y1_use[csub] - y2_use[clo_ind:chi_ind])^2) + $
                    ((z1_use[csub] - z2_use[clo_ind:chi_ind])^2), min_sub)
...
if (nsubs GT 0L) then begin
  for i = 0L,(nsubs - 1L) do begin
    ...
    clo_ind = lo_ind[csub]
    chi_ind = hi_ind[csub]
    min_dist2 = min(((x2_use[csub] - x1_use[clo_ind:chi_ind])^2) + ((y2_use[csub] - y1_use[clo_ind:chi_ind])^2) + $
                    ((z2_use[csub] - z1_use[clo_ind:chi_ind])^2), min_sub)
