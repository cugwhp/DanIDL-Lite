FUNCTION test_bytintlon_scalar, num
...
;               of the type BYTE, INTEGER or LONG would have.
;
; Return Value:
;
;   The function returns an INTEGER value set to "1" if "num" contains a scalar number of the type BYTE,
;   INTEGER, or LONG, and set to "0" if "num" does not contain a scalar number of the type BYTE, INTEGER,
;   or LONG.
;
...
;If "num" is scalar (single) and contains a number of the type BYTE, INTEGER, or LONG, then return a value
;of "1", otherwise return a value of "0"
type = result[1]
if ((type GT 0L) AND (type LT 4L)) then return, 1
return, 0
