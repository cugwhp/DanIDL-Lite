FUNCTION test_intlonfltdbl_scalar, num
...
;               of the type INTEGER, LONG, FLOAT, or DOUBLE would have.
;
; Return Value:
;
;   The function returns an INTEGER value set to "1" if "num" contains a scalar number of the type INTEGER,
;   LONG, FLOAT, or DOUBLE, and set to "0" if "num" does not contain a scalar number of the type INTEGER,
;   LONG, FLOAT, or DOUBLE.
;
...
;If "num" is scalar (single) and contains a number of the type INTEGER, LONG, FLOAT, or DOUBLE, then return
;a value of "1", otherwise return a value of "0"
type = result[1]
if ((type GT 1L) AND (type LT 6L)) then return, 1
return, 0
